...
use vixikhd\skywars\task\SlapperNameTops;
...
use vixikhd\skywars\provider\ArenaManager;
...
class SkyWars extends PluginBase implements Listener {
    ...
    public function onEnable() {
        ...
        Entity::registerEntity(EntityTops::class, true);
        foreach ($this->getResources() as $resource) {
            $this->saveResource($resource->getFilename());
        }
        $this->getServer()->getCommandMap()->register("SkyWars", $this->commands[] = new SkyWarsCommand($this));
        $this->getScheduler()->scheduleRepeatingTask(new SlapperNameTops($this), 20);
    ...
    public function addTops( float $x, float $y, float $z, Level $level, Player $sender)
    {
        foreach ($level->getEntities() as $entity) {
            if ($entity instanceof EntityTops) {
                $id = $entity->getSkin()->getSkinId();
                if ($id === "swtops") {
                    $entity->kill();
                }
            }
        ...
        //$text = $this->getFFAText($modalidad);
        $nbt = Entity::createBaseNBT($vector);
        $nbt->setTag($sender->namedtag->getTag('Skin'));
        $human = new EntityTops($level, $nbt);
        $human->setSkin(new Skin("swtops", $human->getInvisibleSkin()));
        $human->setNameTagVisible(true);
        $human->setNameTagAlwaysVisible(true);
        $human->setNameTag("swtops");
        $human->setGenericFlag(Entity::DATA_FLAG_NO_AI);
    ...
    public function noDmgTOPS(EntityDamageEvent $ev)
    {
